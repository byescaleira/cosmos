...
- [[ci-no-cross-builds]] — risk (0.11.0): CI runs only host `swift build` + `swift test` — no per-platform `--triple` cross-build matrix, so platform-absent-symbol regressions (e.g. `glassEffect` unavailable on visionOS, used ungated in `CosmosButton` + `cosmosToast`) land on `main` green. The visionOS regression is fixed in 0.11.0 (`#if os(visionOS)` → opaque surface/capsule fallback); the CI gap stays open — guard with the local per-platform build loop until a CI matrix exists.